...
public class VerificationTokenServiceImplTest {
    ...
    public void testGetVerificationToken1() {
        when(tokenRepo.findAllByUserId(testUser.getId())).thenReturn(Optional.of(testToken));
    ...
    public void testGetVerificationToken2() {
        when(tokenRepo.findAllByUserId(testUser.getId())).thenReturn(Optional.empty());
        Assertions.assertThrows(ElementNotFoundException.class, () -> tokenService.getVerificationToken(testUser), "No verification tokens exist for the user");
    }
